...
if(isset($_POST['tell']))
{
		...
		if($m>150)
		{
			$msg="<font color=green>پیام ارسال شد</font>";
			$res=sendSms("http://www.postgah.net/API/SendSms.ashx","golshan","2225","30002223332115",$_POST['tell'],$_POST['text']);
...
$q=sprintf("INSERT INTO `pardakhtiha` ( `email` , `mablagh` , `tarikh` , `transid` , `idget` )VALUES 
				('%s', '%s', '%s', '%s', '%s');",checkInput($_SESSION['useremail']),'-150',getCurrentDate(),'sms','');
				//echo $q;
				$db->query($q);
	...
	else
	{
		...
		$msg="<font color=red>موجودی حساب شما کافی نیست</font>";	
	}
}
